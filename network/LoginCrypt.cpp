...
#include "stdafx.h"
//---------------------------------------------------------------------------
TLoginCrypt::TLoginCrypt()
{
	memset(&m_seed, 0, sizeof(m_seed));
}
//---------------------------------------------------------------------------
void TLoginCrypt::Init(PBYTE ps, DWORD k1, DWORD k2, DWORD k3, DWORD seed_key)
{
	memcpy(m_seed, ps, 4);
...
//---------------------------------------------------------------------------
void TLoginCrypt::Encrypt(PBYTE in, PBYTE out, int size)
{
...
//---------------------------------------------------------------------------
void TLoginCrypt::Encrypt_Old(PBYTE in, PBYTE out, int size)
{
    for (int i = 0; i < size; i++)
    {
...
//---------------------------------------------------------------------------
void TLoginCrypt::Encrypt_1_25_36(PBYTE in, PBYTE out, int size)
{
    for (int i = 0; i < size; i++)
    {
        out[i] = in[i] ^ static_cast<unsigned char>(m_key[0]);
