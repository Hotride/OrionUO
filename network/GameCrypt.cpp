...
//---------------------------------------------------------------------------
// static
bool TBlowfishCrypt::m_tables_ready = false;
//---------------------------------------------------------------------------
void TBlowfishCrypt::InitTables()
{
	for (int key_index=0; key_index<CRYPT_GAMEKEY_COUNT; key_index++)
	{
		...
		PBYTE pkey=g_key_table[key_index];
		PBYTE pkey_end=g_key_table[key_index+1];
		...
		{
			DWORD mask=*pkey++;
			if (pkey>=pkey_end) pkey=g_key_table[key_index];
...
//---------------------------------------------------------------------------
void TBlowfishCrypt::RawEncrypt(PDWORD values, int table)
{
...
//---------------------------------------------------------------------------
void TBlowfishCrypt::Encrypt(PBYTE in, PBYTE out, int len)
{
	while (m_stream_pos+len>CRYPT_GAMETABLE_TRIGGER)
	{
		...
		m_table_index = (m_table_index + CRYPT_GAMETABLE_STEP) % CRYPT_GAMETABLE_MODULO;
		memcpy(m_seed, g_seed_table[1][m_table_index][0], CRYPT_GAMESEED_LENGTH);
		m_stream_pos = 0;
		m_block_pos = 0;
	...
	for (int i=0; i<len; i++)
	{
		if (m_block_pos==0)
		{
			...
			RawEncrypt(values, m_table_index);
...
#if UO_FW_SHARD == 1
		BYTE bS = (m_seed[m_block_pos] + 5) & 0xFF;
		BYTE c = (*in++) ^ (~bS);
		*out++ = c;
#else
		BYTE c = (*in++) ^ m_seed[m_block_pos];
		*out++ = c;
#endif
...
//---------------------------------------------------------------------------
void TBlowfishCrypt::Init()
{
	if (!m_tables_ready) InitTables();
...
//---------------------------------------------------------------------------
void TTwofishCrypt::Init(BYTE IP[4])
{
	memcpy(&m_IP, IP, 4);
	...
	ki.key32[0] = ki.key32[1] = ki.key32[2] = ki.key32[3] = m_IP; //0x0100007f or 0x8edb49ad or may be client_ip xor 0x8fdb49d2

	reKey(&ki);

	for(int i = 0; i < 256; i++) m_subData3[i] = i;
...
//---------------------------------------------------------------------------
void TTwofishCrypt::Init_MD5()
{
	if (m_md5 != NULL) delete m_md5;
	m_md5 = new MD5Crypt();
...
//---------------------------------------------------------------------------
void TTwofishCrypt::Encrypt(PBYTE in, PBYTE out, int size)
{
	BYTE tmpBuff[0x100];
...
//---------------------------------------------------------------------------
void TTwofishCrypt::Decrypt(PBYTE in, PBYTE out, int size)
{
