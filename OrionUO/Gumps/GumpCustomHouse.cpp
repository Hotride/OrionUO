    ...
    : CGump(GT_CUSTOM_HOUSE, serial, x, y)
{
    ...
    ParseCustomHouseObjectFileWithCategory<CCustomHouseObjectWall, CCustomHouseObjectWallCategory>(
        m_Walls, g_App.UOFilesPath("walls.txt"));
    ...
    ParseCustomHouseObjectFileWithCategory<CCustomHouseObjectMisc, CCustomHouseObjectMiscCategory>(
        m_Miscs, g_App.UOFilesPath("misc.txt"));
    ParseCustomHouseObjectFile<CCustomHouseObjectStair>(m_Stairs, g_App.UOFilesPath("stairs.txt"));
    ParseCustomHouseObjectFile<CCustomHouseObjectTeleport>(
        m_Teleports, g_App.UOFilesPath("teleprts.txt"));
    ParseCustomHouseObjectFileWithCategory<CCustomHouseObjectRoof, CCustomHouseObjectRoofCategory>(
        m_Roofs, g_App.UOFilesPath("roof.txt"));
    ParseCustomHouseObjectFile<CCustomHouseObjectPlaceInfo>(
        m_ObjectsInfo, g_App.UOFilesPath("suppinfo.txt"));
    ...
    if (id >= ID_GCH_ITEM_IN_LIST)
    {
        ...
        if (Category == -1 && (State == CHGS_WALL || State == CHGS_ROOF || State == CHGS_MISC))
        {
            if (State == CHGS_WALL && index < (int)m_Walls.size() && m_Walls[index].m_Items.size())
                ...
                State == CHGS_ROOF && index < (int)m_Roofs.size() && m_Roofs[index].m_Items.size())
                cliloc = m_Roofs[index].m_Items[0].TID;
            else if (
                State == CHGS_MISC && index < (int)m_Miscs.size() && m_Roofs[index].m_Items.size())
    ...
    if (Category == -1)
    {
        ...
        IFOR (i, startCategory, endCategory)
        {
            ...
            m_DataBox->Add(new CGUIHitBox(
                ID_GCH_ITEM_IN_LIST + (int)i, offsetX, offsetY, dims.Width, dims.Height, true));
...
void CGumpCustomHouse::DrawStairSection()
{
    if (Page >= 0 && Page < (int)m_Stairs.size())
    {
        ...
        IFOR (j, 0, 2)
        {
            ...
            text->CreateTextureW(
                0, g_ClilocManager.Cliloc(g_Language)->GetW(1062113 + (int)j), 30, 90);
                ...
                if (multi != NULL)
                {
                    QFOR(multiObject, multi->m_Items, CMultiObject *)
                    {
                        if (multiObject->IsCustomHouseMulti() &&
                            !(multiObject->State & CHMOF_GENERIC_INTERNAL) &&
    ...
    CGameItem *foundationItem, CMulti *multi, int minZ, int maxZ, int flags)
{
    ...
    QFOR(item, multi->m_Items, CMultiObject *)
    {
        ...
        if (item->IsCustomHouseMulti() &&
            !(item->State & (CHMOF_FLOOR | CHMOF_STAIR | CHMOF_ROOF | CHMOF_FIXTURE)) &&
            item->GetZ() >= minZ && item->GetZ() < maxZ)
        {
            ...
            if (infoCheck.first != -1 && infoCheck.second != -1)
            {
                ...
                if (flags & CHVCF_DIRECT_SUPPORT)
                {
                    ...
                    ((flags & CHVCF_BOTTOM) && info.Bottom) || ((flags & CHVCF_TOP) && info.Top))
                {
    ...
    vector<WISP_GEOMETRY::CPoint2Di> &validatedFloors)
{
    ...
    {
        auto existsInList = [](vector<WISP_GEOMETRY::CPoint2Di> &validatedFloors,
                               const WISP_GEOMETRY::CPoint2Di &testPoint) {
        ...
        {
            const WISP_GEOMETRY::CPoint2Di table[4] = { WISP_GEOMETRY::CPoint2Di(-1, 0),
            ...
            IFOR (i, 0, 4)
            {
                WISP_GEOMETRY::CPoint2Di testPoint(
                                     ...
                                     multiObject = nextMultiObject)
                                {
                                    ...
                                    else
                                    {
                                        if (!(multiObject->State &
                                              (CHMOF_STAIR | CHMOF_ROOF | CHMOF_FLOOR)))
                                            multi->Delete(multiObject);
                                    }
                                }
            ...
            IFOR (j, 0, 2)
            {
                ...
                if (i && !j)
                {
                    for (const WISP_GEOMETRY::CPoint2Di &point : validatedFloors)
                    {
                        ...
                        QFOR(item, multi->m_Items, CMultiObject *)
                        {
                            if (item->IsCustomHouseMulti() && (item->State & CHMOF_FLOOR) &&
                                item->GetZ() >= minZ && item->GetZ() < maxZ)
                    ...
                    IFOR (x, StartPos.X, EndPos.X + 1)
                    {
                        ...
                        IFOR (y, StartPos.Y, EndPos.Y + 1)
                        {
                            ...
                            {
                                if (item->IsCustomHouseMulti() && (item->State & CHMOF_FLOOR) &&
                                    (item->State & CHMOF_VALIDATED_PLACE) &&
                                    !(item->State & CHMOF_INCORRECT_PLACE) &&
                                    item->GetZ() >= minZ && item->GetZ() < maxZ)
                                {
                                    minY = (int)y;
                                    break;
                        ...
                        IFOR (y, minY, maxY)
                        {
                            ...
                            {
                                if (item->IsCustomHouseMulti() && (item->State & CHMOF_FLOOR) &&
                                    (item->State & CHMOF_VALIDATED_PLACE) && item->GetZ() >= minZ &&
                                    item->GetZ() < maxZ)
                                    item->State = item->State & ~CHMOF_INCORRECT_PLACE;
                            }
                        }
