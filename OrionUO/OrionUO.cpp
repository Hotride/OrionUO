	...
	switch (key)
	{
		...
		case VKI_NEW_TARGET_SYSTEM:
		{
			if (value == -1)
				...
				g_ConfigManager.DisableNewTargetSystem = (value == 0); //������ == 0!!! �.�. � ������� ��� Target System enable/disable
...
void COrion::LoadClientStartupConfig()
{
	...
	if (g_ConfigManager.GetMusic())
	{
		if (g_PacketManager.GetClientVersion() >= CV_7000)
			PlayMusic(78);
		else if (g_PacketManager.GetClientVersion() > CV_308Z)//from 4.x the music played is 0, the first one
			PlayMusic(0);
		else
			PlayMusic(8);
	}
