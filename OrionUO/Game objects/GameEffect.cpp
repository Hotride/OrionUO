...
void CGameEffect::Draw(const int &x, const int &y)
{
	...
	if (m_EffectType == EF_MOVING)
	{
		...
		if (moving->FixedDirection)
			g_Orion.DrawStaticArt(objGraphic, m_Color, drawEffectX, drawEffectY - (m_Z * 4));
		else
			g_Orion.DrawStaticArtRotated(objGraphic, m_Color, drawEffectX, drawEffectY - (m_Z * 4), moving->Angle);
	}
	else if (m_EffectType == EF_DRAG)
	{
		CGameEffectDrag *dragEffect = (CGameEffectDrag*)this;

		g_Orion.DrawStaticArt(m_Graphic, m_Color, x - dragEffect->OffsetX, y - dragEffect->OffsetY - (m_Z * 4));
	}
	else
		g_Orion.DrawStaticArt(objGraphic, m_Color, x, y - (m_Z * 4));
