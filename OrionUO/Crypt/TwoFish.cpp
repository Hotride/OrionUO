...
#define Fe32_128(x, R) \
    (MDStab[0][p8(01)[p8(02)[_b(x, R)] ^ b0(SKEY[1])] ^ b0(SKEY[0])] ^ MDStab[1][p8(11)[p8(12)[_b(x, R + 1)] ^ b1(SKEY[1])] ^ b1(SKEY[0])] ^ MDStab[2][p8(21)[p8(22)[_b(x, R + 2)] ^ b2(SKEY[1])] ^ b2(SKEY[0])] ^ MDStab[3][p8(31)[p8(32)[_b(x, R + 3)] ^ b3(SKEY[1])] ^ b3(SKEY[0])])
#define Fe32_192(x, R) \
    (MDStab[0][p8(01)[p8(02)[p8(03)[_b(x, R)] ^ b0(SKEY[2])] ^ b0(SKEY[1])] ^ b0(SKEY[0])] ^ MDStab[1][p8(11)[p8(12)[p8(13)[_b(x, R + 1)] ^ b1(SKEY[2])] ^ b1(SKEY[1])] ^ b1(SKEY[0])] ^ MDStab[2][p8(21)[p8(22)[p8(23)[_b(x, R + 2)] ^ b2(SKEY[2])] ^ b2(SKEY[1])] ^ b2(SKEY[0])] ^ MDStab[3][p8(31)[p8(32)[p8(33)[_b(x, R + 3)] ^ b3(SKEY[2])] ^ b3(SKEY[1])] ^ b3(SKEY[0])])
#define Fe32_256(x, R) \
    (MDStab[0][p8(01)[p8(02)[p8(03)[p8(04)[_b(x, R)] ^ b0(SKEY[3])] ^ b0(SKEY[2])] ^ b0(SKEY[1])] ^ b0(SKEY[0])] ^ MDStab[1][p8(11)[p8(12)[p8(13)[p8(14)[_b(x, R + 1)] ^ b1(SKEY[3])] ^ b1(SKEY[2])] ^ b1(SKEY[1])] ^ b1(SKEY[0])] ^ MDStab[2][p8(21)[p8(22)[p8(23)[p8(24)[_b(x, R + 2)] ^ b2(SKEY[3])] ^ b2(SKEY[2])] ^ b2(SKEY[1])] ^ b2(SKEY[0])] ^ MDStab[3][p8(31)[p8(32)[p8(33)[p8(34)[_b(x, R + 3)] ^ b3(SKEY[3])] ^ b3(SKEY[2])] ^ b3(SKEY[1])] ^ b3(SKEY[0])])

#define GetSboxKey                  \
    DWORD SKEY[4]; /* local copy */ \
    memcpy(SKEY, key->sboxKeys, sizeof(SKEY));
/*----------------------------------------------------------------*/
#elif defined(MIN_KEY)
#define MOD_STRING "(Minimal keying)"
#define Fe32_(x, R) (MDStab[0][p8(01)[_sBox8_(0)[_b(x, R)]] ^ b0(SKEY0)] ^ MDStab[1][p8(11)[_sBox8_(1)[_b(x, R + 1)]] ^ b1(SKEY0)] ^ MDStab[2][p8(21)[_sBox8_(2)[_b(x, R + 2)]] ^ b2(SKEY0)] ^ MDStab[3][p8(31)[_sBox8_(3)[_b(x, R + 3)]] ^ b3(SKEY0)])
...
#define Fe32_(x, R) (MDStab[0][_sBox8_(0)[_b(x, R)]] ^ MDStab[1][_sBox8_(1)[_b(x, R + 1)]] ^ MDStab[2][_sBox8_(2)[_b(x, R + 2)]] ^ MDStab[3][_sBox8_(3)[_b(x, R + 3)]])
#define sbSet(N, i, J, v)      \
    {                          \
        _sBox8_(N)[i + J] = v; \
    }
...
#define sbSet(N, i, J, v)                                      \
    {                                                          \
        _sBox_[N & 2][2 * i + (N & 1) + 2 * J] = MDStab[N][v]; \
    }
#define GetSboxKey
#endif
...
#define Xor32(dst, src, i)                          \
    {                                               \
        ((DWORD*)dst)[i] = ((DWORD*)src)[i] ^ tmpX; \
    }
...
#define F32(res, x, k32)                                                                                                                                                                                                                                             \
    {                                                                                                                                                                                                                                                                \
        ...
        case 2: /* 128-bit keys (optimize for this case) */                                                                                                                                                                                                          \
            res = MDStab[0][p8(01)[p8(02)[b0(t)] ^ b0(k32[1])] ^ b0(k32[0])] ^ MDStab[1][p8(11)[p8(12)[b1(t)] ^ b1(k32[1])] ^ b1(k32[0])] ^ MDStab[2][p8(21)[p8(22)[b2(t)] ^ b2(k32[1])] ^ b2(k32[0])] ^ MDStab[3][p8(31)[p8(32)[b3(t)] ^ b3(k32[1])] ^ b3(k32[0])]; \
        }                                                                                                                                                                                                                                                            \
    }
...
#endif
    {
        ...
        for (i = 0, j = k64Cnt - 1; i < k64Cnt; i++, j--) { /* split into even/odd key dwords */
...
#endif
    {
        for (i = q = 0; i < subkeyCnt / 2; i++, q += SK_STEP) { /* compute round subkeys for PHT */
...
#if ALIGN32
    if ((((int)cipher) & 3) || (((int)key) & 3) || (((int)input) & 3) || (((int)outBuffer) & 3))
        return BAD_ALIGN32;
#endif
#endif

    if (mode == MODE_CFB1) { /* use recursion here to handle CFB, one block at a time */
...
#define StoreBlockE(N)                         \
    {                                          \
        t0 = x[N ^ 2] ^ sk[OUTPUT_WHITEN + N]; \
        ((DWORD*)outBuffer)[N] = Bswap(t0);    \
    }
#endif
        StoreBlockE(0);
        StoreBlockE(1);
