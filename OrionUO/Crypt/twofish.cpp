...
typedef int cdecl
CipherProc(cipherInstance *cipher, keyInstance *key, u8 *input, int inputLen, u8 *outBuffer);
...
#define Fe32_256(x, R)                                                                             \
    (MDStab[0]                                                                                     \
           [p8(01)[p8(02)[p8(03)[p8(04)[_b(x, R)] ^ b0(SKEY[3])] ^ b0(SKEY[2])] ^ b0(SKEY[1])] ^   \
            b0(SKEY[0])] ^                                                                         \
     MDStab                                                                                        \
         [1]                                                                                       \
         [p8(11)[p8(12)[p8(13)[p8(14)[_b(x, R + 1)] ^ b1(SKEY[3])] ^ b1(SKEY[2])] ^ b1(SKEY[1])] ^ \
          b1(SKEY[0])] ^                                                                           \
     MDStab                                                                                        \
         [2]                                                                                       \
         [p8(21)[p8(22)[p8(23)[p8(24)[_b(x, R + 2)] ^ b2(SKEY[3])] ^ b2(SKEY[2])] ^ b2(SKEY[1])] ^ \
          b2(SKEY[0])] ^                                                                           \
     MDStab                                                                                        \
         [3]                                                                                       \
         [p8(31)[p8(32)[p8(33)[p8(34)[_b(x, R + 3)] ^ b3(SKEY[3])] ^ b3(SKEY[2])] ^ b3(SKEY[1])] ^ \
          b3(SKEY[0])])

#define GetSboxKey                                                                                 \
    u32 SKEY[4]; /* local copy */                                                                  \
    memcpy(SKEY, key->sboxKeys, sizeof(SKEY));
/*----------------------------------------------------------------*/
#elif defined(MIN_KEY)
#define MOD_STRING "(Minimal keying)"
#define Fe32_(x, R)                                                                                \
    (MDStab[0][p8(01)[_sBox8_(0)[_b(x, R)]] ^ b0(SKEY0)] ^                                         \
...
#define Fe32_(x, R)                                                                                \
    (MDStab[0][_sBox8_(0)[_b(x, R)]] ^ MDStab[1][_sBox8_(1)[_b(x, R + 1)]] ^                       \
     MDStab[2][_sBox8_(2)[_b(x, R + 2)]] ^ MDStab[3][_sBox8_(3)[_b(x, R + 3)]])
#define sbSet(N, i, J, v)                                                                          \
    {                                                                                              \
        _sBox8_(N)[i + J] = v;                                                                     \
    }
...
*/
#define Fe32_(x, R)                                                                                \
    (_sBox_[0][2 * _b(x, R)] ^ _sBox_[0][2 * _b(x, R + 1) + 1] ^ _sBox_[2][2 * _b(x, R + 2)] ^     \
     _sBox_[2][2 * _b(x, R + 3) + 1])
    ...
    cipherInstance *cipher, keyInstance *key, CONST u8 *input, int inputLen, u8 *outBuffer)
{
    ...
    {                            /* use blockEncrypt here to handle CFB, one block at a time */
        ...
        for (n = 0; n < inputLen; n++)
        {
            ...
            outBuffer[n / 8] =
                (outBuffer[n / 8] & ~bit0) | (ctBit ^ ((((u8 *)x)[0] & 0x80) >> (n & 7)));
            carry = ctBit >> (7 - (n & 7));
            for (i = BLOCK_SIZE / 8 - 1; i >= 0; i--)
            {
                bit = cipher->IV[i] >> 7; /* save next "carry" from shift */
